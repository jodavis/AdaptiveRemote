...
internal class Old_SpeechRecognition : Old_ISpeechRecognition, IDisposable
{
    ...
    private Channel<IRecognizedSpeech> _commandChannel = Channel.CreateUnbounded<IRecognizedSpeech>();

    private readonly CancellationTokenSource _stopCts = new();
    private readonly object _lockObject = new();

    public Old_SpeechRecognition(IOptionsSnapshot<ConversationSettings> settings, ISpeechRecognitionEngine engine, IListeningController listeningController, IGrammarProvider grammarProvider, ILogger<Old_SpeechRecognition> logger)
    {
        _engine = engine;
    ...
    private Task ResetAttentionTask(CancellationToken cancellationToken)
    {
        lock (_lockObject)
        {
            EnsureCanReset(_attentionTcs.Task, nameof(Old_ISpeechRecognition.ListenForAttentionAsync));
    ...
    async Task Old_ISpeechRecognition.ListenForAttentionAsync(CancellationToken cancellationToken)
    {
    ...
    private Task<bool> ResetYesNoTask(CancellationToken cancellationToken)
    {
        lock (_lockObject)
        {
            EnsureCanReset(_yesNoTcs.Task, nameof(Old_ISpeechRecognition.ListenForYesNoAsync));
    ...
    async Task<bool> Old_ISpeechRecognition.ListenForYesNoAsync(CancellationToken cancellationToken)
    {
        cancellationToken = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, _stopCts.Token).Token;
    ...
    private Channel<IRecognizedSpeech> ResetCommandChannel(CancellationToken cancellationToken)
    {
        lock (_lockObject)
        {
            EnsureCanReset(_commandChannel.Reader.Completion, nameof(Old_ISpeechRecognition.ListenForCommandsAsync));
            _commandChannel = Channel.CreateBounded<IRecognizedSpeech>(new BoundedChannelOptions(_settings.CommandBufferSize)
            {
                SingleReader = true,
    ...
    IAsyncEnumerable<IRecognizedSpeech> Old_ISpeechRecognition.ListenForCommandsAsync(CancellationToken cancellationToken)
    {
        cancellationToken = CancellationTokenSource.CreateLinkedTokenSource(cancellationToken, _stopCts.Token).Token;

        Channel<IRecognizedSpeech> resultChannel = ResetCommandChannel(cancellationToken);

        _ = StartAndStopListeningToChannelAsync(resultChannel);

        return resultChannel.Reader.ReadAllAsync(cancellationToken);

        async Task StartAndStopListeningToChannelAsync(Channel<IRecognizedSpeech> resultChannel)
        {
            try
            {
                await StartAndStopListeningAsync(_commandsGrammar, resultChannel.Reader.Completion, nameof(Old_ISpeechRecognition.ListenForCommandsAsync));
            }
            catch (Exception ex)
            {
