...
- **Key Pattern:** Command name (e.g., `Power`, `VolumeUp`, `VolumeDown`) under `[IRData]` section
...
- Only commands present in the `[IRData]` section are considered programmed and enabled in programming mode.
- The file can be pre-populated for development or testing (e.g., `Settings.sample.ini`).

## Notes
- The file may contain other sections or keys, but only keys in the `[IRData]` section are used for IR payloads.
